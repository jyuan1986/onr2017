...
for train, test in rkf.split(X):
  ...
  ytr_rbf = svr_rbf.fit(X_train, y_train).predict(X_train)
  ytt_rbf = svr_rbf.fit(X_train, y_train).predict(X_test)
  y_rbf   = svr_rbf.fit(X_train, y_train).predict(tcts)
  lden1.append(y_rbf) 
...
plt.errorbar(dens, mden1, sden1,color='navy',marker='s', label='RBF model',linestyle='None')
fin=np.concatenate((dens.reshape(-1,1),mden1.reshape(-1,1),sden1.reshape(-1,1)),axis=1)
np.savetxt('SVR-RBF-dens-vs-prediction.txt', fin, delimiter='\t')
plt.xlabel('%Mass Change (experiment)')
plt.ylabel('%Mass Change (SVR Prediction)')
plt.legend()
